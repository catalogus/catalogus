/**
 * Migration Script: Move WordPress featured images to Supabase Storage
 *
 * Usage:
 *   export SUPABASE_URL="your-project-url"
 *   export SUPABASE_SERVICE_ROLE_KEY="your-service-role-key"
 *   npx tsx scripts/migrate-post-featured-images.ts --dry-run --limit=25 --match=/wp-content/uploads/
 *
 * Notes:
 * - Keeps original dimensions (no resize).
 * - Converts to WebP when possible; falls back to original bytes for GIF/SVG.
 * - Updates featured_image_url + featured_image_path in posts.
 */
...
const buildTargetPath = (
  ...
  return `post-images/${postId}/${Date.now()}-${base}.${safeExtension}`
}
...
const migrateImage = async (postId: string, url: string) => {
  const sourceName = basename(new URL(url).pathname) || `image-${Date.now()}`
  ...
  if (shouldConvertToWebp(contentType, sourceName)) {
    try {
      ...
      console.warn(`   ⚠️  WebP conversion failed, keeping original bytes: ${error}`)
    }
  }

  const path = buildTargetPath(postId, sourceName, extension)
  if (dryRun) {
...
async function main() {
  console.log('\n🚀 Starting Post Featured Image Migration\n')
  ...
  const baseQuery = supabase
    .from('posts')
    .select('id, featured_image_url, featured_image_path', { count: 'exact' })
    .not('featured_image_url', 'is', null)
    .ilike('featured_image_url', `%${match}%`)

  const { count } = await baseQuery
  const total = count ?? 0
  console.log(`\n📊 Found ${total} candidate posts\n`)
      ...
      try {
        ...
        if (!dryRun) {
          const { error: updateError } = await supabase
            .from('posts')
            .update({
              featured_image_url: migrated.publicUrl,
              featured_image_path: migrated.path,
            })
            .eq('id', row.id)
